# -*- coding: utf-8 -*-
"""v7 빌드: 인터미션 계열(대화+메뉴/파라미터 라벨) 미보유 음절 + ADV 잔존 3자 보충.
베이스 v5 → ACE3_KR_v7.iso. 모든 변경이 등길이 제자리(재패킹 없음). 247(v6) 상위판.

배경(2026-07-14 사용자 제보 2차: 출격 선택 메뉴 '？험치(경험치)'·'발동 ？건(조건)'·
'？니어(버니어)' — v6이 <operator( 문자열만 고쳐 메뉴 라벨이 남음):
- 인터미션 렌더 폰트 8783/8784는 자기 엔트리 안에 UI 문자열 사본을 갖고 그걸
  자기 폰트로 그린다('발동 조건'이 8780/8783/8787 각각에 사본으로 실존).
  → 리매핑 범위 = ①<operator( 문자열 전부 ②8783/8784 엔트리의 전 문자열
    ③<operator( 문자열을 가진 시나리오 엔트리의 전 문자열.
    단 라디오 태그(<op(/<sp(/<on() 문자열은 제외 — 미션 캡션 폰트로 렌더됨.
- 결손 코드 census도 위 범위 전체로 계산(메뉴 전용 음절이 op 42종 밖에 있을 수 있음).
- 글리프: 8780/8782/8787과 8783/8784는 kr 글리프 박스·픽셀 분포가 완전 동일(실측)
  → 8780계에서 비트맵 그대로 복사, 셸에도 없는 희귀 음절은 render_fit 폴백.
  풀은 전 db 문자열(미번역 일본어 포함) 미사용 코드만 사용 — 한자 라벨 무손상.
- ADV(4614)+대형6 공통 풀을 확장 범위(0xE0xx)에서 3개 추가 발견(E0F3/E56A/E8A6,
  전부 16x18+) → 사용자 제보 음절 뗄·혜 + 잔존 최빈 깁 배정.
입력: ACE3_KR_v5.iso → 출력: ACE3_KR_v7.iso
"""
...
OUT_ISO = Path(os.environ.get("ACE3_V7_OUT", str(W / "ACE3_KR_v7.iso")))
...
def main():
    ...
    # ---------- 스코프 결정(1차: <operator( 보유 엔트리) ----------
    op_entries = set()
    for sec in db:
        if sec["idx"] in op_entries:
            continue
        ...
        for off in tbl:
            if off == 0 or off >= len(cur):
                continue
            if OP in gs(cur, off):
                op_entries.add(sec["idx"])
                break
    SCOPE_EIDX = op_entries | set(SHELL_DST)
    print(f"인터미션 스코프 엔트리 {len(SCOPE_EIDX)}개 (op {len(op_entries)} + 8783/8784)")

    def in_scope(sec, s):
        """8783/8784 폰트로 렌더되는 문자열: op 태그 문자열 전부 +
        스코프 엔트리의 전 문자열(단 라디오 태그는 미션 캡션 렌더라 제외)."""
        if OP in s:
            return True
        return sec["idx"] in SCOPE_EIDX and not any(t in s for t in RADIO_TAGS)

    # ---------- 전역 사용 코드 + 스코프 미보유 census ----------
    used_global = Counter()
    scope_codes = Counter()
    for sec in db:
        cur, tbl = read_sec(sec)
    ...
    missing_op = {c: n for c, n in scope_codes.items()
                  if any(gi_of(Fdst[e], c) is None for e in SHELL_DST)}
    print(f"스코프 미보유 코드 {len(missing_op)}종 / {sum(missing_op.values())}회:",
          "".join(sorted(kr_inv[c] for c in missing_op if c in kr_inv)))
    ...
    for sec in db:
        ...
        if changed:
            ...
            n_sec += 1
    print(f"스코프 문자열 재작성: {n_sec}섹션 {n_str}건")
    ...
    for sec in db:
        ...
        for off in {o for o in tbl if 0 < o < len(cur)}:
            ...
            for _p, sz, code in iter_units(s):
                if sz != 2:
                    continue
                if is_sc and any(gi_of(Fck[e], code) is None for e in SHELL_DST):
                    left_op[code] += 1
                if is_tag and gi_of(Fck[ADV], code) is None:
                    left_tag[code] += 1
    print(f"검증: 스코프 잔존 미보유 {sum(left_op.values())}회 {len(left_op)}종",
